...
_EXE_ASSET = (
    "SwornTweaks.exe" if platform.system() == "Windows" else "SwornTweaks-Linux"
)
...
def find_game_path() -> Path | None:
    ...
    if platform.system() == "Windows":
        for drive in "CDEFGH":
            candidates.append(
                rf"{drive}:\Program Files (x86)\Steam\steamapps\common\SWORN"
            )
...
class UpdateChecker(QThread):
    ...
    def run(self):
        try:
            ...
            req = urllib.request.Request(
                GITHUB_CONFIGURATOR, headers={"User-Agent": "SwornTweaks"}
            )
...
class Configurator(QMainWindow):
    def __init__(self):
        ...
        # Blessings groups
        self._rerolls_group = self._group(
            "Rerolls",
            [
        ...
        self._rarity_group = self._group(
            "Blessing Rarity",
            [
        ...
        self._extra_blessings_group = self._group(
            "Extra Blessings",
            [
        ...
        # Toggles groups
        self._toggles_group = self._group(
            "General",
            [
        ...
        self._fae_group = self._group(
            "Guaranteed Fae Realms",
            [
                self._bool_row("GuaranteedFaeKiss", "Guaranteed Kiss Portal"),
                self._bool_row(
                    "GuaranteedFaeKissCurse", "Guaranteed Kiss Curse Portal"
                ),
            ],
        ...
        self._save_state_group = self._group(
            ...
            [
                ...
                self._label_row(
                    "Saves run state (biome, room, health, blessings) after each room."
                ),
                self._bool_row("LoadSaveOnStart", "Load Save on Next Run"),
                self._label_row(
                    "Resume from save on the next run start. Auto-resets after load."
                ),
            ],
        ...
        self._sword_group = self._group(
            "Guaranteed Sword in the Stone",
            [
                sword_row,
                self._int_row(
                    "GuaranteedSwordsBiomes", "Total Swords in the Stone", 0, 4
                ),
            ],
        ...
        # Player group
        self._player_group = self._group(
            "Player Scaling",
            [
                self._float_row(
                    "PlayerHealthMultiplier", "Player Health", 0.1, 50.0, "x"
                ),
                self._float_row(
                    "PlayerDamageMultiplier", "Player Damage", 0.1, 50.0, "x"
                ),
        ...
        # Enemies groups
        self._boss_scaling_group = self._group(
            "Boss Scaling",
            [
                self._float_row("BossHealthMultiplier", "Boss Health", 0.1, 50.0, "x"),
                self._float_row("BossDamageMultiplier", "Boss Damage", 0.1, 50.0, "x"),
                self._float_row(
                    "BeastHealthMultiplier", "Beast Health", 0.1, 50.0, "x"
                ),
                self._float_row(
                    "BeastDamageMultiplier", "Beast Damage", 0.1, 50.0, "x"
                ),
            ],
            "Scale boss and beast health/damage multipliers.",
        )

        self._enemy_scaling_group = self._group(
            "Enemy Scaling",
            [
                self._float_row(
                    "EnemyHealthMultiplier", "Enemy Health", 0.1, 50.0, "x"
                ),
                self._float_row(
                    "EnemyDamageMultiplier", "Enemy Damage", 0.1, 50.0, "x"
                ),
                self._label_row(
                    "Affects normal enemies only.\nBoss/beast health has its own multipliers."
                ),
            ],
            "Scale normal enemy health/damage.\nDoes not affect bosses or beasts.",
        )

        self._intensity_group = self._group(
            "Spawn Intensity",
            [
                self._float_row(
                    "IntensityMultiplier", "Enemy Spawn Rate", 0.1, 10.0, "x"
                ),
        ...
        bd_lay.addWidget(
            self._label_row("Include legendary beasts in extra boss room pool.")
        )
        ...
        bd_lay.addWidget(
            self._label_row(
                "Guaranteed extra boss rooms per biome.\n"
        ...
        bd_lay.addWidget(
            self._label_row("Max random beasts per biome (excludes fixed).")
        )
        self._beast_details.setVisible(False)

        self._extra_boss_group = self._group(
            "Extra Bosses",
            [enable_boss_row, self._beast_details],
        ...
        self._fixed_bosses_cb.stateChanged.connect(
            lambda _: self._update_beast_enables()
        )
        ...
        fb_lay.addLayout(
            self._float_row("FightBossDamageMultiplier", "Boss Damage", 0.1, 10.0, "x")
        )
        ...
        self._fight_boss_combo.currentIndexChanged.connect(
            lambda _: self._update_boss_hp_hint()
        )
        self._update_boss_hp_hint()
        self._fight_boss_details.setVisible(False)

        self._fight_boss_group = self._group(
            "Fight a Specific Boss",
            [fight_boss_row, self._fight_boss_details],
        ...
        self._fight_boss_cb.stateChanged.connect(
            lambda _: self._update_fight_boss_enables()
        )
        ...
        run_rows.append(
            self._label_row(
                "Adds more biomes before camelot.\nIn order if not randomized."
            )
        )
        run_rows.append(
            self._bool_row("RandomizeRepeats", "Randomize Repeated Biomes Order")
        )
        ...
        growth_row = self._float_row(
            "ProgressiveScalingGrowth", "HP Scaling Growth", 1.0, 3.0, "x"
        )
        self._growth_label = growth_row.itemAt(0).widget()
        run_rows.append(growth_row)
        run_rows.append(
            self._label_row(
                "Scales difficulty for extra or random biomes.\n1.0 = no scaling."
            )
        )
        self._run_length_group = self._group(
            "More Biomes/Rooms",
            run_rows,
        ...
        self.widgets["AllBiomesRandom"].stateChanged.connect(
            lambda _: self._update_extra_enables()
        )
        self.widgets["ProgressiveScaling"].stateChanged.connect(
            lambda _: self._update_extra_enables()
        )
        ...
        rd_lay.addLayout(
            self._int_row("BossRushExtraBlessings", "Extra Blessings per Room", 0, 3)
        )
        rd_lay.addLayout(
            self._int_row("BossRushHealPerRoom", "Player HP Heal per Room", 0, 100)
        )
        rd_lay.addLayout(
            self._float_row(
                "BossRushScaling", "Boss HP Scaling per Room", 1.0, 3.0, "x"
            )
        )
        ...
        slay.addSpacing(4)
        gh_label = QLabel(
            f'GitHub &amp; Install Instructions: <a href="https://github.com/{GITHUB_REPO}/blob/main/INSTALL.md">'
        ...
        s_export_cfg_btn.setToolTip(
            "Save a copy of the config file to a chosen location"
        )
        ...
        logs_btn.setToolTip(
            "Open MelonLoader logs folder — attach Latest.log to bug reports"
        )
        ...
        help_sections = [
            (
                "Reporting Bugs",
                "Found a bug? Click the yellow 'Report Bug' button above to open a structured "
            ...
            (
                "Player",
                "Adjust player health and damage multipliers, toggle infinite mana or invincibility. "
                "Set bonus rerolls, tweak blessing rarity percentages, and add extra blessings per room.",
            ),
            (
                "Enemies",
                "Scale boss, beast, and regular enemy health and damage independently. "
                "Adjust enemy spawn intensity per room.",
            ),
            (
                "Toggles",
                "Quick on/off switches for quality-of-life features: free gems, Ring of Dispel unlock, "
                "unlimited gold, skip Somewhere, guaranteed Fae Realm portals, and Sword in the Stone.",
            ),
            (
                "Game Modes",
                "Boss Rush: fight all bosses back-to-back with configurable scaling and rewards. "
            ...
            (
                "Settings",
                "Update the mod and configurator from GitHub, reset to vanilla defaults, "
    ...
    def _update_boss_hp_hint(self):
        ...
        self._fight_boss_hp_hint.setText(
            f"0 = use default boss health (default: {hp:,} on Squire difficulty)"
        )
    ...
    def _load_from_cfg(self, path: Path | None):
        ...
        for key, widget in self.widgets.items():
            ...
            if isinstance(widget, QCheckBox):
                val = (
                    raw.lower() in ("true", "1", "yes") if raw is not None else default
                )
        ...
        vanilla_val = (
            vanilla_raw.lower() in ("true", "1", "yes")
            if vanilla_raw is not None
            else VANILLA_DEFAULTS["UseVanillaBeastSettings"]
        )
        ...
        fb_val = (
            fb_raw.lower() in ("true", "1", "yes")
            if fb_raw is not None
            else VANILLA_DEFAULTS["FightBossMode"]
        )
        ...
        fb_sel = (
            fb_sel_raw.strip('"')
            if fb_sel_raw is not None
            else VANILLA_DEFAULTS["FightBossSelection"]
        )
    ...
    def _save(self):
        ...
        cfg.set(
            SECTION,
            "UseVanillaBeastSettings",
            str(not self._enable_bosses_cb.isChecked()).lower(),
        )

        # Fight Boss manual controls
        cfg.set(SECTION, "FightBossMode", str(self._fight_boss_cb.isChecked()).lower())
        cfg.set(
            SECTION, "FightBossSelection", f'"{self._fight_boss_combo.currentData()}"'
        )
    ...
    def _report_bug():
        ...
        QDesktopServices.openUrl(
            QUrl(f"https://github.com/{GITHUB_REPO}/issues/new?template=bug_report.yml")
        )
    ...
    def _open_mod_folder(self):
        ...
        if mods and mods.is_dir():
            ...
            QMessageBox.warning(
                self, "Error", "Mods folder not found. Set the game path first."
            )
    ...
    def _export_cfg(self):
        ...
        dest, _ = QFileDialog.getSaveFileName(
            self, "Export Config", "MelonPreferences.cfg", "Config files (*.cfg)"
        )
        if dest:
            import shutil
    ...
    def _import_cfg(self):
        ...
        src, _ = QFileDialog.getOpenFileName(
            self, "Import Config", "", "Config files (*.cfg);;All files (*)"
        )
        if src:
            ...
            self._update_fight_boss_enables()
            QMessageBox.information(
                self, "Imported", "Config loaded. Click Save Config to write to disk."
            )
    ...
    def _paste_code(self):
        ...
        if not text.startswith("ST1:"):
            QMessageBox.warning(
                self,
                "Invalid Code",
                "Clipboard doesn't contain a valid SwornTweaks config code.\n\nCodes start with 'ST1:'.",
            )
            return
        try:
            ...
            QMessageBox.warning(
                self, "Invalid Code", "Could not decode the config code."
            )
            return
        pairs = {}
        ...
        if "_enable_bosses" in pairs:
            ...
            self._enable_bosses_cb.setChecked(
                pairs["UseVanillaBeastSettings"] not in ("1", "true", "True")
            )
        if "_fixed" in pairs:
        ...
        self._update_fight_boss_enables()
        QMessageBox.information(
            self, "Applied", "Config code applied. Click Save .cfg to write to disk."
        )
    ...
    def _check_and_update(self):
        ...
        if not self.mods_path.is_dir():
            QMessageBox.warning(
                self, "Error", f"Mods folder not found:\n{self.mods_path}"
            )
            return
    ...
    def _on_check_failed(self, err: str):
        ...
        QMessageBox.critical(
            self, "Update Check Failed", f"Could not reach GitHub:\n{err}"
        )
    ...
    def _on_already_up_to_date(self):
        ...
        QMessageBox.information(
            self, "Up to Date", f"You already have the latest version (v{VERSION})."
        )
    ...
    def _check_update_complete(self):
        """Called after each download finishes. Acts when all are done."""
        ...
        configurator_updated = (
            "exe" in self._update_results or "cfg" in self._update_results
        )

        if not configurator_updated:
            # DLL-only update — no need to restart
            self.statusBar().showMessage("DLL updated", 5000)
            QMessageBox.information(
                self,
                "DLL Updated",
                "SwornTweaks.dll has been updated.\n\nNo configurator restart needed.",
            )
        elif IS_FROZEN and platform.system() == "Windows":
